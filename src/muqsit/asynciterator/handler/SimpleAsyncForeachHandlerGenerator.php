 ...
 */
final class SimpleAsyncForeachHandlerGenerator implements AsyncForeachHandler{
	...
	private function cancelNext() : void{
		$this->callbacks = [];
		$this->as(static function($key, $value) : Generator{
			return Await::promise(static function ($resolve) {
				$resolve(AsyncForeachResult::CANCEL());
			});
		});
	}
			...
			Await::f2c(function() {
				...
				while($this->iterable->valid()){
					...
					foreach($this->callbacks as $callback){
						if(!(yield from $callback->callAsync($key, $value))->handle($this)){
							return false;
						}
					}
